...
function warmUp() {
  ...
  model.predict(fakeFeatures, false).then(() => {
    console.info(
      `${modelName}: Warm up inference took: ${Date.now() - fakeStart}`
    );
    modelReady = true;
...
async function initTensorflowWASM() {
  if (tf.getBackend() != "wasm") {
    ...
    tf.ready()
      ...
      .catch(() => {
        console.error(
          `tfjs WASM could NOT be initialized, defaulting to ${tf.getBackend()}`
        );
        return false;
      });
  }
...
function modelPredict(features) {
  if (modelReady) {
    ...
    model.predict(features, true).then((predictions) => {
      ...
      const results = summarizedPredictions.filter(
        (_, i) => modelTagOrder[modelName][i]
      )[0];

      console.info(
        `${modelName}: Inference took: ${Date.now() - inferenceStart}`
      );
